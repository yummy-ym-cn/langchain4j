...
public class ObVectorFilterMapper {
    ...
    private static String mapGreaterThanOrEqual(IsGreaterThanOrEqualTo isGreaterThanOrEqualTo) {
        return format(
                "%s >= %s",
                formatKey(isGreaterThanOrEqualTo.key(), isGreaterThanOrEqualTo.comparisonValue()
                        .getClass()), formatValue(isGreaterThanOrEqualTo.comparisonValue()));
    }
    ...
    private static String mapLessThanOrEqual(IsLessThanOrEqualTo isLessThanOrEqualTo) {
        return format("%s <= %s",
                formatKey(isLessThanOrEqualTo.key(), isLessThanOrEqualTo.comparisonValue().getClass()),
                formatValue(isLessThanOrEqualTo.comparisonValue()));
    }

    private static String mapIn(IsIn isIn) {
        return format("%s in %s", formatKey(isIn.key(), String.class),
                formatValuesAsString(isIn.comparisonValues()));
    }

    private static String mapNotIn(IsNotIn isNotIn) {
        String key = formatKey(isNotIn.key(), String.class);
        return format("%s is null or %s not in %s", key, key,
                formatValuesAsString(isNotIn.comparisonValues()));
    }
